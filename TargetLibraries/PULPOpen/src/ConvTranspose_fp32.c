...
__attribute__((noinline, optnone)) void PULP_ConvTranspose2d_s8_s8_s32_CHW(
    const int8_t *__restrict__ pSrcA, uint32_t C_in, uint32_t H_in,
    uint32_t W_in, const int8_t *__restrict__ pSrcB, uint32_t C_out,
    uint32_t groups, uint32_t K_h, uint32_t K_w, uint32_t stride_h,
    uint32_t stride_w, uint32_t dilation_h, uint32_t dilation_w,
    uint32_t pad_top, uint32_t pad_bottom, uint32_t pad_left,
    uint32_t pad_right, const int32_t *__restrict__ pSrcBias,
    const bool has_bias, int32_t *__restrict__ pDstC, uint32_t H_out,
    uint32_t W_out) {
  ...
  for (uint32_t cout = ch_out_start; cout < ch_out_stop; ++cout) {
    int32_t *out_ptr = pDstC + cout * output_plane;
    int32_t init = has_bias ? pSrcBias[cout] : 0;
    for (uint32_t idx = 0; idx < output_plane; ++idx) {
      out_ptr[idx] = init;
    }
  }
  ...
  for (uint32_t cout = ch_out_start; cout < ch_out_stop; ++cout) {
    ...
    for (uint32_t cin_in_group = 0; cin_in_group < ch_in_per_group;
      ...
      for (uint32_t h_in = 0; h_in < H_in; ++h_in) {
        for (uint32_t w_in = 0; w_in < W_in; ++w_in) {
          int32_t val = pSrcA[(cin * H_in + h_in) * W_in + w_in];
