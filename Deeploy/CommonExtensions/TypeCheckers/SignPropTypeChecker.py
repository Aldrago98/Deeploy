...
class SignPropTypeChecker(NodeTypeChecker):
    ...
    def typeInferOutput(self, ctxt: NetworkContext, node: gs.Node,
        ...
        if signProp:
            ...
            for obj, nl, sign in zip(outputs, nLevels, signedness):
                obj.nLevels = nl
                obj._signed = sign
                print(f"  --> Set output {obj.name} nLevels={nl}, _signed={sign}")
        else:
            print(f"  [WARNING] signProp check failed: skipping inference")
